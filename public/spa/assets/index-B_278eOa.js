const __vite__mapDeps = (
  ...
  d = m.f ||
    (m.f = [
      ...
      'assets/index-tQNF5XC1.js',
      'assets/store-ClNN51pB.js',
    ]),
) => i.map((i) => d[i]);
import { cD as _, cE as g, _ as i, cF as y, cG as A, au as p } from './index-CCbaqAw2.js';
import { u as v } from './store-BS_MHMXS.js';
var I = ((t) => (
  (t.All = 'all'),
  (t.Audios = 'audios'),
...
class R {
  ...
  onActionClick = async (e) => {
    const {
      ...
      { useFileStore: w } = await i(
        async () => {
          const { useFileStore: r } = await import('./index-CLJgVhVS.js');
      ...
      { useKnowledgeBaseStore: o } = await i(
        async () => {
          const { useKnowledgeBaseStore: r } = await import('./index-tQNF5XC1.js');
      ...
      { isChunkingUnsupported: h } = await i(
        async () => {
          const { isChunkingUnsupported: r } = await import('./store-BS_MHMXS.js').then((u) => u.e);
      ...
      case 'delete': {
        if (c === 'all' && n.length === 0 && l.queryParams) {
          const { resourceService: u } = await i(
            async () => {
              const { resourceService: S } = await import('./store-BS_MHMXS.js').then((d) => d.h);
  ...
  resolveSelectedResourceIds = async () => {
    ...
    const { resourceService: a } = await i(
        async () => {
          const { resourceService: o } = await import('./store-BS_MHMXS.js').then((h) => h.h);
      ...
      { useFileStore: c } = await i(
        async () => {
          const { useFileStore: o } = await import('./index-CLJgVhVS.js');
