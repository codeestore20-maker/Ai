    ...
    for (let u = 0; u < e.length; ++u) {
      ...
      for (let b = 0; b < r.parentIndex.length; ++b)
        if (h.parentIndex.includes(r.parentIndex[b])) {
          const x = t[r.parentIndex[b]],
            M = Math.atan2(r.x - x.x, r.y - x.y),
            A = Math.atan2(h.x - x.x, h.y - x.y);
          let T = A - M;
          T < 0 && (T += 2 * Math.PI);
          const S = A - T / 2;
          let g = q(y, { x: x.x + x.radius * Math.sin(S), y: x.y + x.radius * Math.cos(S) });
          (g > x.radius * 2 && (g = x.radius * 2),
            (d == null || d.width > g) &&
              (d = { circle: x, width: g, p1: r, p2: h, large: g > x.radius, sweep: !0 }));
        }
...
function le(t, n, s, e) {
  ...
  for (let u = 0; u < i; ++u) {
    ...
    const r = n + h,
      y = t(r);
    if ((y * a >= 0 && (n = r), Math.abs(h) < o || y === 0)) return r;
  }
  return n + h;
}
function ct(t) {
...
function zt(t, n, s) {
  ...
  function M(v) {
    for (let l = 0; l < v.length; l++) x[b][l] = v[l];
    x[b].fx = v.fx;
  }
  const A = (v, l) => v.fx - l.fx,
    T = n.slice(),
    S = n.slice(),
    g = n.slice(),
    m = n.slice();
  for (let v = 0; v < e; ++v) {
    if ((x.sort(A), s.history)) {
      ...
      (p.sort((f, N) => f.id - N.id), s.history.push({ x: x[0].slice(), fx: x[0].fx, simplex: p }));
    }
    d = 0;
    for (let p = 0; p < b; ++p) d = Math.max(d, Math.abs(x[0][p] - x[1][p]));
    if (Math.abs(x[0].fx - x[b].fx) < a && d < c) break;
    for (let p = 0; p < b; ++p) {
      T[p] = 0;
      for (let f = 0; f < b; ++f) T[p] += x[f][p];
      T[p] /= b;
    }
    const l = x[b];
    if ((J(S, 1 + h, T, -h, l), (S.fx = t(S)), S.fx < x[0].fx))
      ...
      if (
        (S.fx > l.fx
          ? (J(g, 1 + r, T, -r, l), (g.fx = t(g)), g.fx < l.fx ? M(g) : (p = !0))
          : (J(g, 1 - r * h, T, r * h, l), (g.fx = t(g)), g.fx < S.fx ? M(g) : (p = !0)),
        p)
      ) {
        if (y >= 1) break;
        for (let f = 1; f < x.length; ++f) (J(x[f], 1 - y, x[0], y, x[f]), (x[f].fx = t(x[f])));
      }
    } else M(S);
  }
...
function ue(t, n, s) {
  ...
  for (let r = 0; r < u; ++r) {
    if (
      ((h = ce(t, a, e, i, h)),
      s.history &&
        s.history.push({ x: e.x.slice(), fx: e.fx, fxprime: e.fxprime.slice(), alpha: h }),
      !h)
    )
      ft(a, e.fxprime, -1);
    else {
...
function At(t, n = {}) {
  ...
  const s = n.initialLayout || ge,
    ...
    a = Object.keys(o),
    c = [];
  for (const r of a) (c.push(o[r].x), c.push(o[r].y));
  const u = zt(
    (r) => {
      const y = {};
      for (let d = 0; d < a.length; ++d) {
        const b = a[d];
        y[b] = { x: r[2 * d], y: r[2 * d + 1], radius: o[b].radius };
      }
      return e(y, i);
    },
    c,
...
const Rt = 1e-10;
function ht(t, n, s) {
  return Math.min(t, n) * Math.min(t, n) * Math.PI <= s + Rt
    ? Math.abs(t - n)
    : le((e) => xt(t, n, e) - s, 0, t + n);
}
function fe(t, n = {}) {
  const s = n.distinct,
    ...
    for (const h of e)
      for (let u = 0; u < h.sets.length; u++) {
        ...
        for (let y = u + 1; y < h.sets.length; y++) {
          const d = String(h.sets[y]),
            b = `${r};${d}`,
            x = `${d};${r}`;
          (c.set(b, h.size + (c.get(b) || 0)), c.set(x, h.size + (c.get(x) || 0)));
        }
      }
    for (const h of e) h.sets.length < 3 && (h.size = c.get(i(h.sets)));
  }
  const o = [],
    a = new Set();
  for (const c of e)
    if (c.sets.length === 1) o.push(c.sets[0]);
  ...
  return (
    t
      ...
      .forEach((o) => {
        const a = s[o.sets[0]],
          c = s[o.sets[1]],
          h = Math.sqrt(n[a].size / Math.PI),
          u = Math.sqrt(n[c].size / Math.PI),
          r = ht(h, u, o.size);
        e[a][c] = e[c][a] = r;
        let y = 0;
        (o.size + 1e-10 >= Math.min(n[a].size, n[c].size) ? (y = 1) : o.size <= 1e-10 && (y = -1),
          (i[a][c] = i[c][a] = y));
      }),
...
function xe(t, n = {}) {
  ...
  let u = null;
  for (let d = 0; d < s; ++d) {
    const b = ct(o.length * 2).map(Math.random),
      x = ue(h, b, n);
    (!u || x.fx < u.fx) && (u = x);
  }
  const r = u.x,
  ...
  for (let d = 0; d < e.length; ++d) {
    const b = e[d];
    y[b.sets[0]] = { x: r[2 * d] * c, y: r[2 * d + 1] * c, radius: Math.sqrt(b.size / Math.PI) };
  }
  if (n.history) for (const d of n.history) ft(d.x, c);
  return y;
}
function ye(t, n) {
  ...
  for (const y of t) {
    ...
    const b = y.sets[0],
      x = y.sets[1];
    (y.size + Rt >= Math.min(e[b].size, e[x].size) && (d = 0),
      i[b].push({ set: x, size: y.size, weight: d }),
      i[x].push({ set: b, size: y.size, weight: d }));
  }
  const o = [];
  ...
  for (let y = 1; y < o.length; ++y) {
    ...
    for (var r = 0; r < b.length; ++r) {
      const S = e[b[r].set],
        g = ht(x.radius, S.radius, b[r].size);
      ...
      for (let m = r + 1; m < b.length; ++m) {
        const v = e[b[m].set],
          l = ht(x.radius, v.radius, b[m].size),
          p = Tt({ x: S.x, y: S.y, radius: g }, { x: v.x, y: v.y, radius: l });
        M.push(...p);
      }
    }
    let A = 1e50,
...
function pe(t, n, s) {
  if ((s == null ? t.sort((i, o) => o.radius - i.radius) : t.sort(s), t.length > 0)) {
    const i = t[0].x,
      o = t[0].y;
    for (const a of t) ((a.x -= i), (a.y -= o));
  }
  if (
    (t.length === 2 &&
      q(t[0], t[1]) < Math.abs(t[1].radius - t[0].radius) &&
...
function dt(t) {
  const n = (s) => {
    const e = t.reduce((o, a) => Math.max(o, a[s] + a.radius), Number.NEGATIVE_INFINITY),
      i = t.reduce((o, a) => Math.min(o, a[s] - a.radius), Number.POSITIVE_INFINITY);
    return { max: e, min: i };
  };
...
function be(t = {}) {
  ...
  function l(p) {
    ...
    function j(k) {
      if (k.sets in U) return U[k.sets];
    ...
    const V = E.selectAll('.venn-area').data(f, (k) => k.sets),
      O = V.enter()
        .append('g')
        .attr(
          'class',
      ...
      W = O.append('text')
        ...
        .attr('y', e / 2);
    r &&
      (B.style('fill-opacity', '0')
        .filter((k) => k.sets.length == 1)
        .style('fill', (k) => (k.colour ? k.colour : k.color ? k.color : g(k.sets)))
    ...
    let z = p;
    _ && typeof z.transition == 'function'
      ? ((z = K(p)), z.selectAll('path').attrTween('d', P))
      : z.selectAll('path').attr('d', (k) => St(k.sets.map((F) => I[F])), x);
    const L = z
      .selectAll('text')
      .filter((k) => k.sets in C)
      .text((k) => j(k))
      .attr('x', (k) => Math.floor(C[k.sets].x))
      .attr('y', (k) => Math.floor(C[k.sets].y));
    ...
    const X = D.selectAll('text')
      .attr('x', s / 2)
      .attr('y', e / 2);
    return (
      y !== null &&
        (W.style('font-size', '0px'), L.style('font-size', y), X.style('font-size', '0px')),
      { circles: I, textCentres: C, nodes: V, enter: O, update: z, exit: D }
    );
  }
  return (
    (l.wrap = function (p) {
    ...
    for (; (u = a.pop()), !!u; ) {
      ...
      ((M.textContent = g),
        g.length > h &&
          ...
          (r.pop(), (M.textContent = r.join(' ')), (r = [u]), (M = x(u)), y++));
    }
    const A = 0.35 - (y * d) / 2,
      T = e.getAttribute('x'),
      S = e.getAttribute('y');
...
function Lt(t, n, s) {
  ...
  for (const r of t)
    (e.push({ x: r.x, y: r.y }),
      e.push({ x: r.x + r.radius / 2, y: r.y }),
      e.push({ x: r.x - r.radius / 2, y: r.y }),
      e.push({ x: r.x, y: r.y + r.radius / 2 }),
      e.push({ x: r.x, y: r.y - r.radius / 2 }));
  let i = e[0],
    o = at(e[0], t, n);
  ...
  const a = zt((r) => -1 * at({ x: r[0], y: r[1] }, t, n), [i.x, i.y], {
      maxIterations: 500,
      minErrorDelta: 1e-10,
    }).x,
    c = { x: s ? 0 : a[0], y: a[1] };
  ...
  for (let e = 0; e < s.length; e++) {
    ...
    for (let a = e + 1; a < s.length; ++a) {
      const c = s[a],
        h = t[c],
        u = q(o, h);
      u + h.radius <= o.radius + 1e-10
        ? n[c].push(i)
        : u + o.radius <= h.radius + 1e-10 && n[i].push(c);
    }
  }
  return n;
}
function jt(t, n, s) {
  ...
  for (let o = 0; o < n.length; ++o) {
    ...
    for (let d = 0; d < a.length; ++d) {
      c[a[d]] = !0;
      const b = i[a[d]];
      for (let x = 0; x < b.length; ++x) h[b[x]] = !0;
    }
    const u = [],
      r = [];
    for (let d in t) d in c ? u.push(t[d]) : d in h || r.push(t[d]);
    const y = Lt(u, r, s);
    ((e[a] = y),
      y.disjoint &&
        n[o].size > 0 &&
...
function Me(t, n = {}) {
  ...
  i && (x = Dt(x, o, a));
  const M = Nt(x, c, h, u, r),
    A = jt(M, t, y),
    T = new Map(
      Object.keys(M).map((m) => [m, { set: m, x: M[m].x, y: M[m].y, radius: M[m].radius }]),
    ),
    S = t.map((m) => {
      const v = m.sets.map((f) => T.get(f)),
        l = Pt(v),
        p = Vt(l, b);
  ...
  function g(m) {
    let v = '';
    for (const l of S) l.has.size > m.length && m.every((p) => l.has.has(p)) && (v += ' ' + l.path);
    return v;
  }
...
var gt = (function () {
  var t = w(function (S, g, m, v) {
    ...
    M = {
      ...
      performAction: w(function (g, m, v, l, p, f, N) {
        var I = f.length - 1;
        switch (p) {
          case 1:
            return f[I - 1];
          ...
          case 10:
            ...
            break;
          case 11:
            (l.addSubsetData([f[I - 2]], void 0, parseFloat(f[I])),
              l.setIndentMode && l.setIndentMode(!0));
            break;
          case 12:
            (l.addSubsetData([f[I - 3]], f[I - 2], parseFloat(f[I])),
              l.setIndentMode && l.setIndentMode(!0));
            break;
          case 13:
            if (f[I].length < 2) throw new Error('union requires multiple identifiers');
            (l.validateUnionIdentifiers && l.validateUnionIdentifiers(f[I]),
              l.addSubsetData(f[I], void 0, void 0),
              l.setIndentMode && l.setIndentMode(!0));
            break;
          case 14:
            if (f[I - 1].length < 2) throw new Error('union requires multiple identifiers');
            (l.validateUnionIdentifiers && l.validateUnionIdentifiers(f[I - 1]),
              l.addSubsetData(f[I - 1], f[I], void 0),
              l.setIndentMode && l.setIndentMode(!0));
            break;
          case 15:
            if (f[I - 2].length < 2) throw new Error('union requires multiple identifiers');
            (l.validateUnionIdentifiers && l.validateUnionIdentifiers(f[I - 2]),
              l.addSubsetData(f[I - 2], void 0, parseFloat(f[I])),
              l.setIndentMode && l.setIndentMode(!0));
            break;
          case 16:
            if (f[I - 3].length < 2) throw new Error('union requires multiple identifiers');
            (l.validateUnionIdentifiers && l.validateUnionIdentifiers(f[I - 3]),
              l.addSubsetData(f[I - 3], f[I - 2], parseFloat(f[I])),
              l.setIndentMode && l.setIndentMode(!0));
            break;
          case 17:
      ...
      table: [
        ...
        t(s, [2, 21]),
        { 20: c, 26: 51 },
        { 16: u, 20: r, 21: [1, 53], 28: 52, 29: 54, 30: 55, 31: y, 32: d, 33: b },
        t(s, [2, 12]),
        t(s, [2, 16]),
        t(h, [2, 30]),
        t(h, [2, 31]),
        t(h, [2, 32]),
        t(h, [2, 33], { 30: 61, 16: u, 20: r, 31: y, 32: d, 33: b }),
        t(x, [2, 34]),
        t(x, [2, 36]),
        t(x, [2, 37]),
        t(x, [2, 38]),
      ...
      parse: w(function (g) {
        ...
        function K() {
          ...
          return (
            (G = l.pop() || _.lex() || E),
            typeof G != 'number' &&
              (G instanceof Array && ((l = G), (G = l.pop())), (G = m.symbols_[G] || G)),
            G
          );
        }
        w(K, 'lex');
        for (var z, L, D, X, k = {}, F, H, et, Y; ; ) {
          if (
            ...
            this.defaultActions[L]
              ? (D = this.defaultActions[L])
              : ((z === null || typeof z > 'u') && (z = K()), (D = N[L] && N[L][z])),
            typeof D > 'u' || !D.length || !D[0])
          ) {
            var Z = '';
            Y = [];
            for (F in N[L]) this.terminals_[F] && F > j && Y.push("'" + this.terminals_[F] + "'");
            (_.showPosition
              ? (Z =
                  'Parse error on line ' +
                  (C + 1) +
...
Expecting ` +
              ...
              : (Z =
                  ...
                  (C + 1) +
                  ': Unexpected ' +
                  (z == E ? 'end of input' : "'" + (this.terminals_[z] || z) + "'")),
              this.parseError(Z, {
                text: _.match,
          ...
          if (D[0] instanceof Array && D.length > 1)
            throw new Error(
              'Parse Error: multiple actions possible at state: ' + L + ', token: ' + z,
            );
          switch (D[0]) {
            case 1:
              (v.push(z),
            ...
            case 2:
              if (
                ...
                B && (k._$.range = [f[f.length - (H || 1)].range[0], f[f.length - 1].range[1]]),
                (X = this.performAction.apply(k, [I, U, C, P.yy, D[1], p, f].concat(R))),
                typeof X < 'u')
              )
                return X;
              (H &&
                ((v = v.slice(0, -1 * H * 2)), (p = p.slice(0, -1 * H)), (f = f.slice(0, -1 * H))),
                v.push(this.productions_[D[1]][0]),
                p.push(k.$),
                f.push(k._$),
                (et = N[v[v.length - 2]][v[v.length - 1]]),
                v.push(et));
              break;
            case 3:
              return !0;
        ...
        unput: w(function (g) {
          ...
          return (
            (this.yylloc = {
              ...
              last_column: v
                ? (v.length === l.length ? this.yylloc.first_column : 0) +
                  l[l.length - v.length].length -
                  v[0].length
                : this.yylloc.first_column - m,
            }),
            this.options.ranges && (this.yylloc.range = [p[0], p[0] + this.yyleng - m]),
...
` +
        ...
        next: w(function () {
          ...
          for (var p = this._currentRules(), f = 0; f < p.length; f++)
            if (
              ((v = this._input.match(this.rules[p[f]])), v && (!m || v[0].length > m[0].length))
            ) {
              if (((m = v), (l = f), this.options.backtrack_lexer)) {
                if (((g = this.test_match(v, p[f])), g !== !1)) return g;
                if (this._backtrack) {
                  m = !1;
                  continue;
                } else return !1;
          ...
          return m
            ? ((g = this.test_match(m, p[l])), g !== !1 ? g : !1)
            : this._input === ''
              ? this.EOF
              : this.parseError(
                  'Lexical error on line ' +
...
` +
        ...
        performAction: w(function (m, v, l, p) {
          switch (l) {
            case 0:
              break;
            case 1:
...
var Se = gt,
  ...
  we = w((t, n, s) => {
    ...
    ((vt = e),
      e.length === 1 && bt.add(e[0]),
      yt.push({ sets: e, size: i, label: n ? nt(n) : void 0 }));
  }, 'addSubsetData'),
  _e = w(() => yt, 'getSubsetData'),
  nt = w((t) => {
    const n = t.trim();
    return n.length >= 2 && n.startsWith('"') && n.endsWith('"') ? n.slice(1, -1) : n;
  }, 'normalizeText'),
  Te = w((t) => t && nt(t), 'normalizeStyleValue'),
  Ee = w((t, n, s) => {
    const e = nt(n);
...
var Be = w((t, n, s, e) => {
  const i = e.db,
    o = i.getConfig?.(),
    { themeVariables: a, look: c, handDrawnSeed: h } = wt(),
    u = c === 'handDrawn',
    r = [a.venn1, a.venn2, a.venn3, a.venn4, a.venn5, a.venn6, a.venn7, a.venn8].filter(Boolean),
    y = i.getDiagramTitle?.(),
    d = i.getSubsetData(),
    b = i.getTextData(),
    x = qt(i.getStyleData()),
    ...
    S = M / 1600,
    g = y ? 48 * S : 0,
    m = a.primaryTextColor ?? a.textColor,
    v = Jt(n);
  (v.attr('viewBox', `0 0 ${M} ${A}`),
    y &&
      v
        .append('text')
        .text(y)
        ...
        .attr('x', '50%')
        .attr('y', 32 * S)
        .style('fill', a.vennTitleTextColor || a.titleColor));
  const l = ot(document.createElement('div')),
    p = be()
      .width(M)
      .height(A - g);
  l.datum(d).call(p);
  const f = u ? Qt.svg(l.select('svg').node()) : void 0,
    N = Me(d, { width: M, height: A - g, padding: o?.padding ?? 15 }),
    I = new Map();
  for (const E of N) {
  ...
  (l.selectAll('.venn-circle').each(function (E, R) {
    const _ = ot(this),
      V = Q([...E.sets].sort()),
      O = x.get(V),
      B = O?.fill || r[R % r.length] || a.primaryColor;
    _.classed(`venn-set-${R % 8}`, !0);
    const W = O?.['fill-opacity'] ?? 0.1,
      K = O?.stroke || B,
      z = O?.['stroke-width'] || `${5 * S}`;
    if (u && f) {
      const D = I.get(V);
      if (D && D.circles.length > 0) {
        const X = D.circles[0],
          k = f.circle(X.x, X.y, X.radius * 2, {
            roughness: 0.7,
            seed: h,
            fill: kt(B, 0.7),
            fillStyle: 'hachure',
    ...
    u && f
      ? l.selectAll('.venn-intersection').each(function (E) {
          const R = ot(this),
            P = Q([...E.sets].sort()),
            V = x.get(P),
            O = V?.fill;
          ...
          R.select('text')
            .style('font-size', `${48 * S}px`)
            .style('fill', V?.color ?? a.vennSetTextColor ?? m);
        })
      : (l
  ...
  const U = v.append('g').attr('transform', `translate(0, ${g})`),
    j = l.select('svg').node();
  if (j && 'childNodes' in j) for (const E of [...j.childNodes]) U.node()?.appendChild(E);
...
function Ut(t, n, s, e, i, o) {
  ...
  for (const [r, y] of u.entries()) {
    ...
    const b = d.text.x,
      x = d.text.y,
      M = Math.min(...d.circles.map((E) => E.radius)),
      A = Math.min(...d.circles.map((E) => E.radius - Math.hypot(b - E.x, x - E.y)));
    ...
    const g = Math.max(80 * i, T * 2 * 0.95),
      m = Math.max(60 * i, T * 2 * 0.95),
      p =
        (d.data.label && d.data.label.length > 0 ? Math.min(32 * i, T * 0.25) : 0) +
        (y.length <= 2 ? 30 * i : 0),
      f = b - g / 2,
      N = x - m / 2 + p,
      I = Math.max(1, Math.ceil(Math.sqrt(y.length))),
      C = Math.max(1, Math.ceil(y.length / I)),
      U = g / I,
      j = m / C;
...
var qe = { draw: Be },
  en = { parser: Se, db: je, renderer: qe, styles: Ve };
