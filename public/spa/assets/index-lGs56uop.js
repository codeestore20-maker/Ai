const __vite__mapDeps = (
  ...
  d = m.f ||
    (m.f = [
      ...
      'assets/index-BPXzzhUa.js',
      'assets/store-2_FS7oYO.js',
    ]),
) => i.map((i) => d[i]);
import { cD as _, cE as g, _ as i, cF as y, cG as A, au as p } from './index-wqF_YeCD.js';
import { u as v } from './store-I8paDSMM.js';
var I = ((t) => (
  (t.All = 'all'),
  (t.Audios = 'audios'),
...
class R {
  ...
  onActionClick = async (e) => {
    const {
      ...
      { useFileStore: w } = await i(
        async () => {
          const { useFileStore: r } = await import('./index-C8yxPHFg.js');
      ...
      { useKnowledgeBaseStore: o } = await i(
        async () => {
          const { useKnowledgeBaseStore: r } = await import('./index-BPXzzhUa.js');
      ...
      { isChunkingUnsupported: h } = await i(
        async () => {
          const { isChunkingUnsupported: r } = await import('./store-I8paDSMM.js').then((u) => u.e);
      ...
      case 'delete': {
        if (c === 'all' && n.length === 0 && l.queryParams) {
          const { resourceService: u } = await i(
            async () => {
              const { resourceService: S } = await import('./store-I8paDSMM.js').then((d) => d.h);
  ...
  resolveSelectedResourceIds = async () => {
    ...
    const { resourceService: a } = await i(
        async () => {
          const { resourceService: o } = await import('./store-I8paDSMM.js').then((h) => h.h);
      ...
      { useFileStore: c } = await i(
        async () => {
          const { useFileStore: o } = await import('./index-C8yxPHFg.js');
