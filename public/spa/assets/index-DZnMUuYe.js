import { t as z, f as x } from './index-DulcjDYi.js';
...
class R {
  ...
  async serializeEventStream({ eventStream: s, requestSchema: n, initialRequest: o }) {
    const c = this.marshaller,
      y = n.getEventStreamMember(),
      S = n.getMemberSchema(y),
      d = this.serializer,
    ...
    return c.serialize(p, (r) => {
      if (r[h]) return { headers: r.headers, body: r.body };
      const t = Object.keys(r).find((i) => i !== '__type') ?? '',
        {
          additionalHeaders: e,
          body: a,
          eventType: l,
          explicitPayloadContentType: m,
        } = this.writeEventBody(t, S, r);
      return {
        headers: {
  ...
  async deserializeEventStream({ response: s, responseSchema: n, initialResponseContainer: o }) {
    const c = this.marshaller,
      y = n.getEventStreamMember(),
      d = n.getMemberSchema(y).getMemberSchemas(),
      u = Symbol('initialResponseMarker'),
      h = c.deserialize(s.body, async (t) => {
        ...
        if (e === 'initial-response') {
          ...
          if (l.isStructSchema()) {
            const m = {};
            let f = !1;
            for (const [i, g] of l.structIterator()) {
              const { eventHeader: v, eventPayload: w } = g.getMergedTraits();
              if (((f = f || !!(v || w)), w))
                g.isBlobSchema()
                  ? (m[i] = a)
                  : g.isStringSchema()
                    ? (m[i] = (this.serdeContext?.utf8Encoder ?? z)(a))
                    : g.isStructSchema() && (m[i] = await this.deserializer.read(g, a));
              else if (v) {
                const b = t[e].headers[i]?.value;
                b != null &&
                  (g.isNumericSchema()
                    ? b && typeof b == 'object' && 'bytes' in b
                      ? (m[i] = BigInt(b.toString()))
                      : (m[i] = Number(b))
                    : (m[i] = b));
              }
            }
            if (f) return { [e]: m };
            if (a.byteLength === 0) return { [e]: {} };
          }
          return { [e]: await this.deserializer.read(l, a) };
        } else return { $unknown: t };
      }),
      p = h[Symbol.asyncIterator](),
      r = await p.next();
    if (r.done) return h;
    if (r.value?.[u]) {
      if (!n)
      ...
      for (const [t, e] of Object.entries(r.value)) o[t] = e;
    }
    return {
      async *[Symbol.asyncIterator]() {
        for (r?.value?.[u] || (yield r.value); ; ) {
          const { done: t, value: e } = await p.next();
          if (t) break;
          yield e;
        }
      },
  ...
  writeEventBody(s, n, o) {
    const c = this.serializer;
    let y = s,
      S = null,
      d;
    const u = n.getSchema()[4].includes(s),
      h = {};
    if (u) {
      const t = n.getMemberSchema(s);
      if (t.isStructSchema()) {
          ...
          if (m) S = e;
            ...
            (a.isNumericSchema()
              ...
              : a.isTimestampSchema()
                ...
                : a.isStringSchema()
                  ? (i = 'string')
                  : a.isBooleanSchema() && (i = 'boolean'),
              f != null && ((h[e] = { type: i, value: f }), delete o[s][e]));
          }
        }
        if (S !== null) {
          ...
          (e.isBlobSchema()
            ...
            : e.isStringSchema() && (d = 'text/plain'),
            c.write(e, o[s][S]));
        } else c.write(t, o[s]);
      } else if (t.isUnitSchema()) c.write(t, {});
      else
        throw new Error(
    ...
    const p = c.flush() ?? new Uint8Array();
    return {
      body: typeof p == 'string' ? (this.serdeContext?.utf8Decoder ?? x)(p) : p,
      eventType: y,
