...
import { l as ee } from './linear-BkEJkjA2.js';
...
var Et = (function () {
  var t = o(function (M, r, l, x) {
    ...
    p = [1, 7],
    y = [
      1, 4, 5, 10, 12, 13, 14, 18, 25, 35, 37, 39, 41, 42, 48, 50, 51, 52, 53, 54, 55, 56, 57, 60,
      61, 63, 64, 65, 66, 67,
    ],
    _ = [
      1, 4, 5, 10, 12, 13, 14, 18, 25, 28, 35, 37, 39, 41, 42, 48, 50, 51, 52, 53, 54, 55, 56, 57,
    ...
    kt = {
      ...
      performAction: o(function (r, l, x, c, S, e, ut) {
        var s = e.length - 1;
        switch (S) {
          case 23:
            this.$ = e[s];
      ...
      table: [
        { 18: n, 26: 1, 27: 2, 28: f, 55: d, 56: h, 57: p },
        { 1: [3] },
        { 18: n, 26: 8, 27: 2, 28: f, 55: d, 56: h, 57: p },
        { 18: n, 26: 9, 27: 2, 28: f, 55: d, 56: h, 57: p },
        t(y, [2, 33], { 29: 10 }),
        t(_, [2, 61]),
        t(_, [2, 62]),
        t(_, [2, 63]),
        ...
        { 4: K, 5: Z, 6: J, 8: $, 11: tt, 13: et, 16: 89, 17: it, 18: at, 19: nt, 20: st, 22: 105 },
        t(R, [2, 26]),
        t(a, [2, 50], { 10: At }),
        t(Kt, [2, 28], {
          16: 103,
      ...
      parse: o(function (r) {
        ...
        function $t() {
          ...
          return (
            (B = c.pop() || E.lex() || Jt),
            typeof B != 'number' &&
              (B instanceof Array && ((c = B), (B = c.pop())), (B = l.symbols_[B] || B)),
            B
          );
        }
        o($t, 'lex');
        for (var w, j, N, vt, rt = {}, Tt, X, te, mt; ; ) {
          if (
            ((j = x[x.length - 1]),
            this.defaultActions[j]
              ? (N = this.defaultActions[j])
              : ((w === null || typeof w > 'u') && (w = $t()), (N = ut[j] && ut[j][w])),
            typeof N > 'u' || !N.length || !N[0])
          ) {
            var Ct = '';
            mt = [];
            for (Tt in ut[j])
              this.terminals_[Tt] && Tt > me && mt.push("'" + this.terminals_[Tt] + "'");
            (E.showPosition
              ? (Ct =
                  'Parse error on line ' +
                  (yt + 1) +
...
Expecting ` +
          ...
          if (N[0] instanceof Array && N.length > 1)
            throw new Error(
              'Parse Error: multiple actions possible at state: ' + j + ', token: ' + w,
            );
          switch (N[0]) {
            case 1:
              (x.push(w),
            ...
            case 2:
              if (
                ...
                be && (rt._$.range = [e[e.length - (X || 1)].range[0], e[e.length - 1].range[1]]),
                (vt = this.performAction.apply(rt, [s, Zt, yt, Y.yy, N[1], S, e].concat(qe))),
                typeof vt < 'u')
              )
                return vt;
              (X &&
                ((x = x.slice(0, -1 * X * 2)), (S = S.slice(0, -1 * X)), (e = e.slice(0, -1 * X))),
                x.push(this.productions_[N[1]][0]),
                S.push(rt.$),
                e.push(rt._$),
                (te = ut[x[x.length - 2]][x[x.length - 1]]),
                x.push(te));
              break;
            case 3:
              return !0;
        ...
        unput: o(function (r) {
          ...
          return (
            (this.yylloc = {
              ...
              last_column: x
                ? (x.length === c.length ? this.yylloc.first_column : 0) +
                  c[c.length - x.length].length -
                  x[0].length
...
` +
        ...
        next: o(function () {
          ...
          for (var S = this._currentRules(), e = 0; e < S.length; e++)
            if (
              ((x = this._input.match(this.rules[S[e]])), x && (!l || x[0].length > l[0].length))
            ) {
              if (((l = x), (c = e), this.options.backtrack_lexer)) {
                if (((r = this.test_match(x, S[e])), r !== !1)) return r;
                if (this._backtrack) {
                  l = !1;
                  continue;
                } else return !1;
          ...
          return l
            ? ((r = this.test_match(l, S[c])), r !== !1 ? r : !1)
            : this._input === ''
              ? this.EOF
              : this.parseError(
                  'Lexical error on line ' +
...
` +
        ...
        performAction: o(function (l, x, c, S) {
          switch (c) {
            case 0:
              break;
            case 1:
        ...
        conditions: {
          ...
          INITIAL: {
            rules: [
              0, 1, 2, 3, 4, 6, 8, 10, 13, 14, 15, 16, 17, 18, 19, 20, 21, 24, 27, 29, 34, 35, 36,
...
var De = Et,
  ...
  ze =
    ((ot = class {
      ...
      setThemeConfig(n) {
        (bt.trace('setThemeConfig called with: ', n),
      ...
      calculateSpace(n, f, d, h) {
        const p = this.config.xAxisLabelPadding * 2 + this.config.xAxisLabelFontSize,
          y = { top: n === 'top' && f ? p : 0, bottom: n === 'bottom' && f ? p : 0 },
          ...
          m = this.config.quadrantPadding + y.top + u.top,
          q = this.config.chartWidth - this.config.quadrantPadding * 2 - a.left - a.right,
          b = this.config.chartHeight - this.config.quadrantPadding * 2 - y.top - y.bottom - u.top,
          g = q / 2,
          G = b / 2;
        return {
          xAxisSpace: y,
            ...
            g.push({
              ...
              x:
                this.config.yAxisPosition === 'left'
                  ...
                  : this.config.yAxisLabelPadding + A + m + this.config.quadrantPadding,
              y: T + a - (b ? _ / 2 : 0),
      ...
      getQuadrants(n) {
        const { quadrantSpace: f } = n,
          { quadrantHalfHeight: d, quadrantLeft: h, quadrantHalfWidth: p, quadrantTop: y } = f,
          _ = [
            {
              text: {
      ...
      getQuadrantPoints(n) {
        const { quadrantSpace: f } = n,
          { quadrantHeight: d, quadrantLeft: h, quadrantTop: p, quadrantWidth: y } = f,
          _ = ee()
            .domain([0, 1])
            .range([h, y + h]),
          a = ee()
    ...
    o(ot, 'QuadrantBuilder'),
    ot),
  lt,
  qt =
    ((lt = class extends Error {
      constructor(n, f, d) {
        (super(`value for ${n} ${f} is invalid, please use a valid ${d}`),
          (this.name = 'InvalidStyleError'));
      }
    }),
    o(lt, 'InvalidStyleError'),
    lt);
function Dt(t) {
...
function St(t) {
  ...
  for (const f of t) {
    ...
    if (d === 'radius') {
      if (ae(h)) throw new qt(d, h, 'number');
      n.radius = parseInt(h);
    } else if (d === 'color') {
      if (Dt(h)) throw new qt(d, h, 'hex code');
      n.color = h;
    } else if (d === 'stroke-color') {
      if (Dt(h)) throw new qt(d, h, 'hex code');
      n.strokeColor = h;
    } else if (d === 'stroke-width') {
      if (ne(h)) throw new qt(d, h, 'number of pixels (eg. 10px)');
      n.strokeWidth = h;
...
function xe(t, n, f, d, h) {
  const p = St(h);
  z.addPoints([{ x: f, y: d, text: Q(t.text), className: n, ...p }]);
}
o(xe, 'addPoint');
function fe(t, n) {
  z.addClass(t, St(n));
...
function ye() {
  const t = zt(),
    { themeVariables: n, quadrantChart: f } = t;
  return (
    f && z.setConfig(f),
    z.setThemeConfig({
...
` + t,
    ...
    let A;
    a === 'sandbox' && (A = Lt('#i' + n));
    const T = (a === 'sandbox' ? Lt(A.nodes()[0].contentDocument.body) : Lt('body')).select(
        `[id="${n}"]`,
      ),
      m = T.append('g').attr('class', 'main'),
      q = _.quadrantChart?.chartWidth ?? 500,
      b = _.quadrantChart?.chartHeight ?? 500;
    (ve(T, b, q, _.quadrantChart?.useMaxWidth ?? !0),
      T.attr('viewBox', '0 0 ' + q + ' ' + b),
      d.db.setHeight(b),
      d.db.setWidth(q));
    const g = d.db.getQuadrantData(),
      G = m.append('g').attr('class', 'quadrants'),
      ht = m.append('g').attr('class', 'border'),
      xt = m.append('g').attr('class', 'data-points'),
      ft = m.append('g').attr('class', 'labels'),
      gt = m.append('g').attr('class', 'title');
    (g.title &&
      gt
        .append('text')
        .attr('x', 0)
        .attr('y', 0)
        .attr('fill', g.title.fill)
        .attr('font-size', g.title.fontSize)
        .attr('dominant-baseline', h(g.title.horizontalPos))
        .attr('text-anchor', p(g.title.verticalPos))
        .attr('transform', y(g.title))
        .text(g.title.text),
      g.borderLines &&
        ht
          .selectAll('line')
    ...
    const ct = G.selectAll('g.quadrant')
      .data(g.quadrants)
      .enter()
      .append('g')
      .attr('class', 'quadrant');
  ...
  Re = { draw: Be },
  ti = { parser: De, db: we, renderer: Re, styles: o(() => '', 'styles') };
export { ti as diagram };
