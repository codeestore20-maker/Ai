...
import { F as d } from './index-NXWXCubP.js';
import { r as i } from '../vendor/vendor-emotion-DdM-9MtU.js';
import { t as D } from './index-DaaCQ1_i.js';
import { S as L } from './NavItem-DzbtAjUo.js';
import { P as C, a as M } from './index-Cbw3UUZQ.js';
import { p as s } from './selectors-4s1jLJiJ.js';
import { u as g } from './store-jt5YvtvF.js';
...
import './selectors-LlGJfBpJ.js';
import './identifier-DNYzSWMy.js';
const I = i.memo(({ searchKeyword: m }) => {
  const a = i.useRef(null),
    c = i.useRef(-1),
    [l, x, u] = g((t) => [s.hasMoreDocuments(t), s.isLoadingMoreDocuments(t), t.loadMoreDocuments]),
    n = g(s.getFilteredDocuments),
    f = i.useMemo(() => {
      if (!m.trim()) return n;
      const t = m.toLowerCase();
      return n.filter((e) => {
        const S = e.content?.toLowerCase() || '',
          j = e.title?.toLowerCase() || '';
        return S.includes(t) || j.includes(t);
      });
    }, [n, m]),
    r = f.length,
    p = m.trim().length > 0,
    h = i.useCallback(async () => {
      ...
      const t = a.current;
      if (!t || !l) return;
      const e = t.findItemIndex(t.scrollOffset + t.viewportSize);
      c.current < r && e + 5 > r && ((c.current = r), await u());
    }, [l, u, r, p]),
    w = x && !p;
  return r === 0
    ? o.jsx(C, { search: p })
    : o.jsxs(D, {
        ...
        children: [
          f.map((t) =>
            o.jsx(d, { gap: 1, padding: '4px 8px', children: o.jsx(M, { pageId: t.id }) }, t.id),
          ),
          w && o.jsx(d, { padding: '4px 8px', children: o.jsx(L, { rows: 3 }) }),
        ],
      });
});
I.displayName = 'AllPagesDrawerContent';
export { I as default };
