...
var J = (function () {
  var e = o(function (T, t, s, i) {
    ...
    D = {
      ...
      performAction: o(function (t, s, i, c, f, n, v) {
        var w = n.length - 1;
        switch (f) {
          case 6:
          case 7:
      ...
      parse: o(function (t) {
        ...
        function X(B) {
          ((i.length = i.length - 2 * B), (f.length = f.length - B), (n.length = n.length - B));
        }
        o(X, 'popStack');
        function tt() {
          var B;
          return (
            (B = c.pop() || b.lex() || A),
            typeof B != 'number' &&
              (B instanceof Array && ((c = B), (B = c.pop())), (B = s.symbols_[B] || B)),
            B
          );
        }
        o(tt, 'lex');
        for (var M, W, N, Y, z = {}, U, V, et, Z; ; ) {
          if (
            ((W = i[i.length - 1]),
            this.defaultActions[W]
              ? (N = this.defaultActions[W])
              : ((M === null || typeof M > 'u') && (M = tt()), (N = v[W] && v[W][M])),
            typeof N > 'u' || !N.length || !N[0])
          ) {
            var q = '';
            Z = [];
            for (U in v[W]) this.terminals_[U] && U > L && Z.push("'" + this.terminals_[U] + "'");
            (b.showPosition
              ? (q =
                  'Parse error on line ' +
                  (I + 1) +
...
Expecting ` +
              ...
              : (q =
                  ...
                  (I + 1) +
                  ': Unexpected ' +
                  (M == A ? 'end of input' : "'" + (this.terminals_[M] || M) + "'")),
              this.parseError(q, {
                text: b.match,
          ...
          switch (N[0]) {
            ...
            case 2:
              if (
                ...
                G && (z._$.range = [n[n.length - (V || 1)].range[0], n[n.length - 1].range[1]]),
                (Y = this.performAction.apply(z, [w, $, I, S.yy, N[1], f, n].concat(C))),
                typeof Y < 'u')
              )
                return Y;
              (V &&
                ((i = i.slice(0, -1 * V * 2)), (f = f.slice(0, -1 * V)), (n = n.slice(0, -1 * V))),
      ...
      var T = {
        ...
        unput: o(function (t) {
          ...
          return (
            ...
            this.options.ranges && (this.yylloc.range = [f[0], f[0] + this.yyleng - s]),
...
` +
        ...
        next: o(function () {
          ...
          for (var f = this._currentRules(), n = 0; n < f.length; n++)
            if (
              ((i = this._input.match(this.rules[f[n]])), i && (!s || i[0].length > s[0].length))
            ) {
              if (((s = i), (c = n), this.options.backtrack_lexer)) {
                if (((t = this.test_match(i, f[n])), t !== !1)) return t;
                if (this._backtrack) {
                  s = !1;
                  continue;
                } else return !1;
          ...
          return s
            ? ((t = this.test_match(s, f[c])), t !== !1 ? t : !1)
            : this._input === ''
              ? this.EOF
              : this.parseError(
                  'Lexical error on line ' +
...
` +
        ...
        performAction: o(function (s, i, c, f) {
          switch (c) {
            case 0:
              return 6;
            case 1:
...
var xt = J,
    ...
    ((H = class {
      ...
      addNode(h, r) {
        ...
        const y = this.stack[this.stack.length - 1].node,
          p = { text: a, children: [] };
        (y.children.push(p), this.stack.push({ level: l, node: p }));
      }
      getAccTitle() {
        return gt();
      }
  ...
  It = o((e, h, r, a) => {
    ...
    if (!y) return;
    const p = ot(),
      { look: d, handDrawnSeed: m, themeVariables: u } = p,
      k = ut(p.fontSize)[0] ?? St,
      g = d === 'handDrawn',
      _ = y.children ?? [],
      x = p.ishikawa?.diagramPadding ?? 20,
      D = p.ishikawa?.useMaxWidth ?? !1,
      O = dt(h),
      E = O.append('g').attr('class', 'ishikawa'),
      T = g ? yt.svg(O.node()) : void 0,
      t = T
        ? {
            roughSvg: T,
            seed: m ?? 0,
    ...
    let i = 0,
      c = F;
    const f = g ? void 0 : j(E, i, c, i, c, 'ishikawa-spine');
    if ((Lt(E, i, c, y.text, k, t), !_.length)) {
      (g && j(E, i, c, i, c, 'ishikawa-spine', t), at(O, x, D));
      return;
    }
    i -= 20;
    const n = _.filter((S, P) => P % 2 === 0),
      v = _.filter((S, P) => P % 2 === 1),
      w = rt(n),
      I = rt(v),
      $ = w.total + I.total;
    let L = F,
    ...
    const C = k * 2;
    ((L = Math.max(L, w.max * C)),
      (A = Math.max(A, I.max * C)),
      (c = Math.max(L, F)),
      f && f.attr('y1', c).attr('y2', c),
      E.select('.ishikawa-head-group').attr('transform', `translate(0,${c})`));
    ...
    for (let S = 0; S < b; S++) {
      ...
      for (const [R, G, X] of [
        ...
        R && Pt(P, R, i, c, G, X, k, t);
      i = P.selectAll('text')
        .nodes()
        .reduce((R, G) => Math.min(R, G.getBBox().x), 1 / 0);
    }
    if (g) j(E, i, c, 0, c, 'ishikawa-spine', t);
    else {
      f.attr('x1', i);
      const S = `url(#${s})`;
  ...
  Lt = o((e, h, r, a, l, y) => {
    const p = Math.max(6, Math.floor(110 / (l * 0.6))),
      d = e
        .append('g')
        .attr('class', 'ishikawa-head-group')
        .attr('transform', `translate(${h},${r})`),
      m = K(d, ct(a, p), 0, 0, 'ishikawa-head-label', 'start', l),
      u = m.node().getBBox(),
      k = Math.max(60, u.width + 6),
      g = Math.max(40, u.height * 2 + 40),
    ...
    const r = [],
      ...
      l = o((y, p, d) => {
        ...
        for (const u of m) {
          const k = r.length,
            g = u.children ?? [];
          (r.push({ depth: d, text: ct(u.text, 15), parentIndex: p, childCount: g.length }),
            d % 2 === 0
              ? (a.push(k), g.length && l(g, k, d + 1))
              : (g.length && l(g, k, d + 1), a.push(k)));
        }
      }, 'walk');
  ...
  Mt = o((e, h, r, a, l, y, p) => {
    const d = e.append('g').attr('class', 'ishikawa-label-group'),
      u = K(d, h, r, a + 11 * l, 'ishikawa-label cause', 'middle', y)
        .node()
        .getBBox();
    if (p) {
      const k = p.roughSvg.rectangle(u.x - 20, u.y - 2, u.width + 40, u.height + 4, {
        roughness: 1.5,
        seed: p.seed,
        fill: p.fillColor,
        fillStyle: 'hachure',
  ...
  Q = o((e, h, r, a, l, y) => {
    ...
    const d = a / p,
      ...
      _ = h,
      x = r,
      D = `M ${_} ${x} L ${_ - d * u * 2 + k} ${x - m * u * 2 + g} L ${_ - d * u * 2 - k} ${x - m * u * 2 - g} Z`,
      O = y.roughSvg.path(D, {
        roughness: 1,
        seed: y.seed,
        fill: y.lineColor,
  ...
  Pt = o((e, h, r, a, l, y, p, d) => {
    const m = h.children ?? [],
      ...
      x = a + g;
    if (
      (j(e, r, a, _, x, 'ishikawa-branch', d),
      d && Q(e, r, a, r - _, a - x, d),
      Mt(e, h.text, _, x, l, p, d),
      !m.length)
    )
      return;
    const { entries: D, yOrder: O } = Tt(m, l),
      E = D.length,
      T = new Array(E);
    for (const [f, n] of O.entries()) T[n] = a + g * ((f + 1) / (E + 1));
    const t = new Map();
    t.set(-1, { x0: r, y0: a, x1: _, y1: x, childCount: m.length, childrenDrawn: 0 });
    const s = -st,
      i = nt * l,
      c = l < 0 ? 'ishikawa-label up' : 'ishikawa-label down';
    for (const [f, n] of D.entries()) {
      const v = T[f],
      ...
      if (n.depth % 2 === 0) {
        const C = w.y1 - w.y0;
        (($ = lt(w.x0, w.x1, C ? (v - w.y0) / C : 0.5)),
          (L = v),
          (A = $ - (n.childCount > 0 ? Et + n.childCount * At : $t)),
          j(I, $, v, A, v, 'ishikawa-sub-branch', d),
          d && Q(I, $, v, 1, 0, d),
          K(I, n.text, A, v, 'ishikawa-label align', 'end', p));
      } else {
        const C = w.childrenDrawn++;
        (($ = lt(w.x0, w.x1, (w.childCount - C) / (w.childCount + 1))),
          (L = w.y0),
          (A = $ + s * ((v - L) / i)),
          j(I, $, L, A, v, 'ishikawa-sub-branch', d),
          d && Q(I, $, L, $ - A, L - v, d),
          K(I, n.text, A, v, c, 'end', p));
      }
      n.childCount > 0 &&
        t.set(f, { x0: $, y0: L, x1: A, y1: v, childCount: n.childCount, childrenDrawn: 0 });
    }
  }, 'drawBranch'),
  Bt = o((e) => e.split(/<br\s*\/?>|\n/), 'splitLines'),
  ct = o((e, h) => {
...
`);
  ...
  j = o((e, h, r, a, l, y, p) => {
    if (p) {
      const d = p.roughSvg.line(h, r, a, l, {
        roughness: 1.5,
        seed: p.seed,
        stroke: p.lineColor,
        strokeWidth: 2,
