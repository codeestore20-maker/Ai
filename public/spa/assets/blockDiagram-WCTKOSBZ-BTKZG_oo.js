import { g as ce } from './chunk-FMBD7UC4-DXdf8TpC.js';
import {
  _ as g,
  H as at,
  d as T,
...
import { G as me } from './graph-Dy_Z7S6Y.js';
import { c as we } from './channel--5rCQMgh.js';
import { l as Le, k as Se } from './step-B6EOmCqb.js';
...
import './_baseUniq-Dbw5_gOv.js';
import './_baseIsEqual-CetQETvx.js';
var yt = (function () {
  var e = g(function (D, m, d, y) {
      for (d = d || {}, y = D.length; y--; d[D[y]] = m);
      return d;
    }, 'o'),
    t = [1, 15],
    a = [1, 7],
    ...
    u = [1, 23],
    b = [1, 24],
    f = [8, 10, 15, 16, 21, 28, 29, 30, 31, 39, 43, 46],
    w = [8, 10, 15, 16, 21, 27, 28, 29, 30, 31, 39, 43, 46],
    v = [1, 49],
    L = {
      ...
      performAction: g(function (m, d, y, k, E, h, W) {
        var p = h.length - 1;
        switch (E) {
          case 4:
          ...
          case 11:
            ...
            break;
          case 12:
            (k.getLogger().debug('Rule: statement: ', h[p]),
              typeof h[p].length == 'number' ? (this.$ = h[p]) : (this.$ = [h[p]]));
            break;
          case 13:
            (k.getLogger().debug('Rule: statement #2: ', h[p - 1]),
              (this.$ = [h[p - 1]].concat(h[p])));
            break;
          case 14:
            (k.getLogger().debug('Rule: link: ', h[p], m),
              (this.$ = { edgeTypeStr: h[p], label: '' }));
            break;
          case 15:
            (k.getLogger().debug('Rule: LABEL link: ', h[p - 3], h[p - 1], h[p]),
              (this.$ = { edgeTypeStr: h[p], label: h[p - 1] }));
            break;
          case 18:
            const O = parseInt(h[p]),
              Z = k.generateId();
          ...
          case 23:
            ...
            break;
          case 24:
            (k.getLogger().debug('Rule: nodeStatement (abc88 node size) ', h[p - 1], h[p]),
              (this.$ = {
                id: h[p - 1].id,
                label: h[p - 1].label,
            ...
            break;
          case 25:
            (k.getLogger().debug('Rule: nodeStatement (node) ', h[p]),
              (this.$ = {
                id: h[p].id,
                label: h[p].label,
                type: k.typeStr2Type(h[p].typeStr),
            ...
            break;
          case 26:
            (k.getLogger().debug('APA123', this ? this : 'na'),
              k.getLogger().debug('COLUMNS: ', h[p]),
              (this.$ = {
                type: 'column-setting',
            ...
            break;
          case 27:
            (k.getLogger().debug('Rule: id-block statement : ', h[p - 2], h[p - 1]),
              k.generateId(),
          ...
          case 28:
            ...
            break;
          case 29:
            (k.getLogger().debug('Rule: node (NODE_ID separator): ', h[p]),
              (this.$ = { id: h[p] }));
            break;
          case 30:
            (k
          ...
          case 31:
            ...
            break;
          case 32:
            (k.getLogger().debug('Rule: dirList: ', h[p - 1], h[p]),
              (this.$ = [h[p - 1]].concat(h[p])));
            break;
          case 33:
            (k.getLogger().debug('Rule: nodeShapeNLabel: ', h[p - 2], h[p - 1], h[p]),
      ...
      table: [
        ...
        { 44: [1, 30] },
        { 47: [1, 31] },
        e(w, [2, 29], { 32: 32, 35: [1, 33], 37: [1, 34] }),
        { 1: [2, 7] },
        e(x, [2, 13]),
        { 26: 35, 31: s },
        { 31: [2, 14] },
      ...
      parseError: g(function (m, d) {
        if (d.recoverable) this.trace(m);
        else {
          var y = new Error(m);
          throw ((y.hash = d), y);
        }
      ...
      parse: g(function (m) {
        ...
        function Tt() {
          ...
          return (
            (Y = k.pop() || z.lex() || st),
            typeof Y != 'number' &&
              (Y instanceof Array && ((k = Y), (Y = k.pop())), (Y = d.symbols_[Y] || Y)),
            Y
          );
        }
        g(Tt, 'lex');
        for (var P, J, K, pt, Q = {}, it, G, Bt, nt; ; ) {
          if (
            ...
            this.defaultActions[J]
              ? (K = this.defaultActions[J])
              : ((P === null || typeof P > 'u') && (P = Tt()), (K = W[J] && W[J][P])),
...
Expecting ` +
              ...
              : (ft =
                  ...
                  (O + 1) +
                  ': Unexpected ' +
                  (P == st ? 'end of input' : "'" + (this.terminals_[P] || P) + "'")),
              this.parseError(ft, {
                text: z.match,
          ...
          if (K[0] instanceof Array && K.length > 1)
            throw new Error(
              'Parse Error: multiple actions possible at state: ' + J + ', token: ' + P,
            );
          switch (K[0]) {
            case 1:
              (y.push(P),
            ...
            case 2:
              if (
                ...
                ne && (Q._$.range = [h[h.length - (G || 1)].range[0], h[h.length - 1].range[1]]),
                (pt = this.performAction.apply(Q, [p, Z, O, q.yy, K[1], E, h].concat(ie))),
                typeof pt < 'u')
              )
                return pt;
              (G &&
                ((y = y.slice(0, -1 * G * 2)), (E = E.slice(0, -1 * G)), (h = h.slice(0, -1 * G))),
                y.push(this.productions_[K[1]][0]),
                E.push(Q.$),
                h.push(Q._$),
                (Bt = W[y[y.length - 2]][y[y.length - 1]]),
                y.push(Bt));
              break;
            case 3:
              return !0;
    ...
    _ = (function () {
      var D = {
        EOF: 1,
        parseError: g(function (d, y) {
          if (this.yy.parser) this.yy.parser.parseError(d, y);
          else throw new Error(d);
        }, 'parseError'),
        setInput: g(function (m, d) {
          return (
            (this.yy = d || this.yy || {}),
            (this._input = m),
        ...
        unput: g(function (m) {
          ...
          return (
            (this.yylloc = {
              ...
              last_column: y
                ? (y.length === k.length ? this.yylloc.first_column : 0) +
                  k[k.length - y.length].length -
                  y[0].length
...
` +
        ...
        test_match: g(function (m, d) {
          ...
          if (
            ...
            (k = m[0].match(/(?:\r\n?|\n).*/g)),
            k && (this.yylineno += k.length),
            (this.yylloc = {
              first_line: this.yylloc.last_line,
              last_line: this.yylineno + 1,
              first_column: this.yylloc.last_column,
              last_column: k
                ? k[k.length - 1].length - k[k.length - 1].match(/\r?\n?/)[0].length
        ...
        next: g(function () {
          ...
          for (var E = this._currentRules(), h = 0; h < E.length; h++)
            if (
              ((y = this._input.match(this.rules[E[h]])), y && (!d || y[0].length > d[0].length))
            ) {
              if (((d = y), (k = h), this.options.backtrack_lexer)) {
                if (((m = this.test_match(y, E[h])), m !== !1)) return m;
                if (this._backtrack) {
                  d = !1;
                  continue;
                } else return !1;
          ...
          return d
            ? ((m = this.test_match(d, E[k])), m !== !1 ? m : !1)
            : this._input === ''
              ? this.EOF
              : this.parseError(
                  'Lexical error on line ' +
...
` +
        ...
        options: {},
        performAction: g(function (d, y, k, E) {
          switch (k) {
            case 0:
            ...
            case 7:
              return (
                (y.yytext = y.yytext.replace(/columns\s+/, '')),
                d.getLogger().debug('COLUMNS (LEX)', y.yytext),
                28
              );
            case 8:
            ...
            case 14:
              return (
                (y.yytext = y.yytext.replace(/space\:/, '')),
                d.getLogger().debug('SPACE NUM (LEX)', y.yytext),
                21
              );
            case 15:
            ...
            case 85:
              return (
                (y.yytext = y.yytext.replace(/^,\s*/, '')),
                d.getLogger().debug('Lex (right): dir:', y.yytext),
                'DIR'
              );
            case 86:
              return (
                (y.yytext = y.yytext.replace(/^,\s*/, '')),
                d.getLogger().debug('Lex (left):', y.yytext),
                'DIR'
              );
            case 87:
              return (
                (y.yytext = y.yytext.replace(/^,\s*/, '')),
                d.getLogger().debug('Lex (x):', y.yytext),
                'DIR'
              );
            case 88:
              return (
                (y.yytext = y.yytext.replace(/^,\s*/, '')),
                d.getLogger().debug('Lex (y):', y.yytext),
                'DIR'
              );
            case 89:
              return (
                (y.yytext = y.yytext.replace(/^,\s*/, '')),
                d.getLogger().debug('Lex (up):', y.yytext),
                'DIR'
              );
            case 90:
              return (
                (y.yytext = y.yytext.replace(/^,\s*/, '')),
                d.getLogger().debug('Lex (down):', y.yytext),
                'DIR'
              );
            case 91:
              return (
                (y.yytext = ']>'),
                d.getLogger().debug('Lex (ARROW_DIR end):', y.yytext),
                this.popState(),
                this.popState(),
                'BLOCK_ARROW_END'
              );
            ...
            case 96:
              return (
                d.getLogger().debug('Lex: START_LINK', y.yytext),
            ...
            case 100:
              return (
                d.getLogger().debug('Lex: Starting string'),
                this.pushState('string'),
                'LINK_LABEL'
              );
            case 101:
            ...
            case 104:
              return (
                d.getLogger().debug('Lex: COLON', y.yytext),
                (y.yytext = y.yytext.slice(1)),
                27
              );
          }
        }, 'anonymous'),
...
var ke = yt,
  ...
  Ee = R(),
  ct = new Map(),
  _e = g((e) => fe.sanitizeText(e, Ee), 'sanitizeText'),
  De = g(function (e, t = '') {
    let a = ct.get(e);
    (a || ((a = { id: e, styles: [], textStyles: [] }), ct.set(e, a)),
      t?.split(Yt).forEach((i) => {
    ...
    for (const r of a) {
      if (
        (typeof s == 'number' &&
          ...
          r.widthInColumns > s &&
          S.warn(`Block ${r.id} width ${r.widthInColumns} exceeds configured column width ${s}`),
        r.label && (r.label = _e(r.label)),
        r.type === 'classDef')
      ) {
        De(r.id, r.css);
        continue;
...
var Rt = 0,
  Ie = g(() => (Rt++, 'id-' + Math.random().toString(36).substr(2, 12) + '-' + Rt), 'generateId'),
  Ce = g((e) => {
  ...
  Re = g(() => [...X.values()], 'getBlocksFlat'),
  ze = g(() => vt || [], 'getBlocks'),
  Ae = g(() => kt, 'getEdges'),
  Me = g((e) => X.get(e), 'getBlock'),
  Fe = g((e) => {
    X.set(e.id, e);
  }, 'setBlock'),
  Pe = g(() => S, 'getLogger'),
  ...
  Ye = {
    getConfig: g(() => at().block, 'getConfig'),
    typeStr2Type: Kt,
    edgeTypeStr2Type: Ut,
...
function ot(e, t, a = 0, i = 0) {
  ...
  if (e.children?.length > 0) {
    for (const f of e.children) ot(f, t);
    const r = ar(e);
    ((l = r.width),
      (s = r.height),
      S.debug('setBlockSizes abc95 maxWidth of', e.id, ':s children is ', l, s));
    for (const f of e.children)
      f.size &&
        (S.debug(
          `abc95 Setting size of children of ${e.id} id=${f.id} ${l} ${s} ${JSON.stringify(f.size)}`,
        ),
        (f.size.width = l * (f.widthInColumns ?? 1) + C * ((f.widthInColumns ?? 1) - 1)),
        (f.size.height = s),
        (f.size.x = 0),
        (f.size.y = 0),
        S.debug(
    ...
    for (const f of e.children) ot(f, t, l, s);
    const n = e.columns ?? -1;
    let c = 0;
    for (const f of e.children) c += f.widthInColumns ?? 1;
    ...
    if (u < a) {
      ...
      const f = (a - x * C - C) / x,
        w = (i - o * C - C) / o;
      (S.debug('Size indata abc88', e.id, 'childWidth', f, 'maxWidth', l),
        S.debug('Size indata abc88', e.id, 'childHeight', w, 'maxHeight', s),
        S.debug('Size indata abc88 xSize', x, 'padding', C));
      for (const v of e.children)
        v.size && ((v.size.width = f), (v.size.height = w), (v.size.x = 0), (v.size.y = 0));
    }
    if (
      (S.debug(
      ...
      if (f > 0) {
        const w = (u - f * C - C) / f;
        S.debug('abc95 (growing to fit) width', e.id, u, e.size?.width, w);
        for (const v of e.children) v.size && (v.size.width = w);
      }
    }
...
function Et(e, t) {
  ...
  const a = e.columns ?? -1;
  if (
    (S.debug('layoutBlocks columns abc95', e.id, '=>', a, e), e.children && e.children.length > 0)
  ) {
    ...
    const r = new Map();
    {
      ...
      for (const b of u) (r.set(b, o), (o += (s.get(b) ?? 0) + C));
    }
    let n = 0;
    S.debug('abc91 block?.size?.x', e.id, e?.size?.x);
    let c = e?.size?.x ? e?.size?.x + (-e?.size?.width / 2 || 0) : -C,
      x = 0;
    for (const o of e.children) {
      const u = e;
      if (!o.size) continue;
      const { width: b, height: f } = o.size,
        { px: w, py: v } = mt(a, n);
      if (
        (v != x &&
          ((x = v),
          (c = e?.size?.x ? e?.size?.x + (-e?.size?.width / 2 || 0) : -C),
          S.debug('New row in layout for block', e.id, ' and child ', o.id, x)),
        S.debug(
          `abc89 layout blocks (child) id: ${o.id} Pos: ${n} (px, py) ${w},${v} (${u?.size?.x},${u?.size?.y}) parent: ${u.id} width: ${b}${C}`,
        ),
        u.size)
      ) {
        const _ = b / 2;
        ...
        const I = r.get(v) ?? 0,
          D = s.get(v) ?? f;
        ((o.size.y = u.size.y - u.size.height / 2 + I + D / 2 + C),
          S.debug(
  ...
  lr = g(async (e, t) => {
    ...
    if (i) {
      ...
      ((c = u.getBoundingClientRect()),
        (x = c),
        b.attr('width', c.width),
        b.attr('height', c.height));
    } else {
      const u = T(n).select('text').node();
      u && typeof u.getBBox == 'function' && (x = u.getBBox());
    }
    (s.attr('transform', $(x, i)), (wt[t.id] = l), (t.width = c.width), (t.height = c.height));
    let o;
    if (t.startLabelLeft) {
      const u = e.insert('g').attr('class', 'edgeTerminals'),
        b = u.insert('g').attr('class', 'inner'),
        f = await U(b, t.startLabelLeft, t.labelStyle);
      o = f;
      let w = f.getBBox();
      if (i) {
        const v = f.children[0],
          L = T(f);
        ((w = v.getBoundingClientRect()), L.attr('width', w.width), L.attr('height', w.height));
      }
      (b.attr('transform', $(w, i)),
        M[t.id] || (M[t.id] = {}),
        (M[t.id].startLeft = u),
    ...
    if (t.startLabelRight) {
      const u = e.insert('g').attr('class', 'edgeTerminals'),
        b = u.insert('g').attr('class', 'inner'),
        f = await U(u, t.startLabelRight, t.labelStyle);
    ...
    if (t.endLabelLeft) {
      const u = e.insert('g').attr('class', 'edgeTerminals'),
        b = u.insert('g').attr('class', 'inner'),
        f = await U(b, t.endLabelLeft, t.labelStyle);
      o = f;
      let w = f.getBBox();
      if (i) {
        const v = f.children[0],
    ...
    if (t.endLabelRight) {
      const u = e.insert('g').attr('class', 'edgeTerminals'),
        b = u.insert('g').attr('class', 'inner'),
        f = await U(b, t.endLabelRight, t.labelStyle);
      o = f;
      let w = f.getBBox();
      if (i) {
        const v = f.children[0],
...
var cr = g((e, t) => {
    S.debug('Moving label abc88 ', e.id, e.label, wt[e.id], t);
    let a = t.updatedPath ? t.updatedPath : t.originalPath;
    const i = R(),
      { subGraphTitleTotalMargin: l } = pe(i);
    if (e.label) {
      const s = wt[e.id];
      let r = e.x,
        n = e.y;
    ...
    if (e.startLabelRight) {
      ...
      let r = e.x,
        n = e.y;
      if (a) {
        const c = tt.calcTerminalLabelPosition(e.arrowTypeStart ? 10 : 0, 'start_right', a);
        ((r = c.x), (n = c.y));
      }
      s.attr('transform', `translate(${r}, ${n})`);
    }
  ...
  node        : x:${e.x} y:${e.y} w:${e.width} h:${e.height}`);
    ...
    if (Math.abs(l - t.y) * r > Math.abs(i - t.x) * c) {
      ...
      n = (o * u) / x;
      const b = { x: a.x < t.x ? a.x + n : a.x - o + n, y: a.y < t.y ? a.y + x - u : a.y - x + u };
      return (
        n === 0 && ((b.x = t.x), (b.y = t.y)),
        o === 0 && (b.x = t.x),
        x === 0 && (b.y = t.y),
        S.debug(`abc89 topp/bott calc, Q ${x}, q ${u}, R ${o}, r ${n}`, b),
        b
      );
    } else {
      a.x < t.x ? (n = t.x - r - i) : (n = i - r - t.x);
      let u = (x * n) / o,
        b = a.x < t.x ? a.x + o - n : a.x - o + n,
        f = a.y < t.y ? a.y + u : a.y - u;
      return (
        S.debug(`sides calc abc89, Q ${x}, q ${u}, R ${o}, r ${n}`, { _x: b, _y: f }),
        n === 0 && ((b = t.x), (f = t.y)),
        o === 0 && (b = t.x),
        x === 0 && (f = t.y),
        { x: b, y: f }
  ...
  gr = g(function (e, t, a, i, l, s, r) {
    ...
    (o?.intersect &&
      ...
      n.unshift(x.intersect(n[0])),
      n.push(o.intersect(n[n.length - 1]))),
      a.toCluster &&
        (S.debug('to cluster abc88', i[a.toCluster]),
        (n = At(a.points, i[a.toCluster].node)),
        (c = !0)),
      a.fromCluster &&
        (S.debug('from cluster abc88', i[a.fromCluster]),
        (n = At(n.reverse(), i[a.fromCluster].node).reverse()),
        (c = !0)));
    const u = n.filter((m) => !Number.isNaN(m.y));
    let b = Se;
    a.curve && (l === 'graph' || l === 'flowchart') && (b = a.curve);
    const { x: f, y: w } = de(a),
      v = Le().x(f).y(w).curve(b);
    let L;
    switch (a.thickness) {
      case 'normal':
    ...
    const _ = e
      ...
      .attr('style', a.style);
    let I = '';
    ((R().flowchart.arrowMarkerAbsolute || R().state.arrowMarkerAbsolute) && (I = ue(!0)),
      ir(_, a, I, r, l));
    let D = {};
...
var fr = qt;
function Jt(e, t, a, i) {
  var l, s, r, n, c, x, o, u, b, f, w, v, L, _, I;
  if (
    ((l = t.y - e.y),
    (r = e.x - t.x),
    (c = t.x * e.y - e.x * t.y),
...
function Qt(e, t, a) {
  ...
  for (var c = i - e.width / 2 - r, x = l - e.height / 2 - n, o = 0; o < t.length; o++) {
    var u = t[o],
      b = t[o < t.length - 1 ? o + 1 : 0],
      f = xr(e, a, { x: c + u.x, y: x + u.y }, { x: c + b.x, y: x + b.y });
    f && s.push(f);
  }
  return s.length
    ? (s.length > 1 &&
...
var br = g((e, t) => {
  ...
  mr = br,
  B = { node: pr, circle: fr, ellipse: Zt, polygon: yr, rect: mr },
  A = g(async (e, t, a, i) => {
    const l = R();
    let s;
    ...
    t.labelText === void 0
      ? (x = '')
      : (x = typeof t.labelText == 'string' ? t.labelText : t.labelText[0]);
    let o;
    t.labelType === 'markdown'
      ...
      : (o = await U(c, Nt(It(x), l), t.labelStyle, !1, i));
    let u = o.getBBox();
    const b = t.padding / 2;
    if (F(l)) {
      ...
      (await ye(f, x),
        ...
        w.attr('width', u.width),
        w.attr('height', u.height));
    }
    return (
      r
      ...
      { shapeSvg: n, bbox: u, halfPadding: b, label: c }
    );
  }, 'labelHelper'),
  N = g((e, t) => {
...
var wr = g(async (e, t) => {
  ...
  H = g((e, t) => `${t || 'node default'}${Mt(e.classes)} ${Mt(e.class)}`, 'getClassesFromNode'),
  Ft = g(async (e, t) => {
    const { shapeSvg: a, bbox: i } = await A(e, t, H(t, void 0), !0),
      l = i.width + t.padding,
  ...
  _r = g(async (e, t) => {
    const { shapeSvg: a, bbox: i } = await A(e, t, H(t), !0),
      l = i.width + t.padding,
      s = i.height + t.padding,
      r = [
        { x: (-2 * s) / 6, y: 0 },
      ...
      (t.intersect = function (o) {
        ...
        if (
          ...
          (Math.abs(b) < t.width / 2 ||
            (Math.abs(b) == t.width / 2 && Math.abs(u.y - t.y) > t.height / 2 - r))
        ) {
          let f = r * r * (1 - (b * b) / (s * s));
...
function ht(e, t, a, i) {
  ...
  (t.includes('t') ? (S.debug('add top border'), s(a)) : r(a),
    t.includes('r') ? (S.debug('add right border'), s(i)) : r(i),
    t.includes('b') ? (S.debug('add bottom border'), s(a)) : r(a),
    t.includes('l') ? (S.debug('add left border'), s(i)) : r(i),
...
var zr = g(async (e, t) => {
    ...
    (typeof n == 'object' ? (c = n[0]) : (c = n),
      S.info('Label text abc79', c, n, typeof n == 'object'));
    ...
    let b = x.getBBox();
    const f = await U(r, u.join ? u.join('<br/>') : u, t.labelStyle, !0, !0);
    if (F(R())) {
      const v = f.children[0],
        L = T(f);
      ((o = v.getBoundingClientRect()), L.attr('width', o.width), L.attr('height', o.height));
    ...
    return (
      T(f).attr(
        'transform',
        'translate( ' +
          (o.width > b.width ? 0 : (b.width - o.width) / 2) +
          ', ' +
          (b.height + w + 5) +
          ')',
      ),
      T(x).attr(
        'transform',
        'translate( ' + (o.width < b.width ? 0 : -(b.width - o.width) / 2) + ', 0)',
      ),
      (o = r.node().getBBox()),
      r.attr('transform', 'translate(' + -o.width / 2 + ', ' + (-o.height / 2 - w + 3) + ')'),
      l
      ...
      s
        ...
        .attr('x1', -o.width / 2 - w)
        .attr('x2', o.width / 2 + w)
        .attr('y1', -o.height / 2 - w + b.height + w)
        .attr('y2', -o.height / 2 - w + b.height + w),
      N(t, l),
  ...
  Mr = g(async (e, t) => {
    const { shapeSvg: a, bbox: i, halfPadding: l } = await A(e, t, H(t, void 0), !0),
  ...
  Fr = g(async (e, t) => {
    ...
    return (
      ...
      c
        ...
        .attr('width', i.width + t.padding)
        .attr('height', i.height + t.padding),
      S.info('DoubleCircle main'),
      N(t, n),
      (t.intersect = function (x) {
        return (
          S.info('DoubleCircle intersect', t, i.width / 2 + l + s, x),
          B.circle(t, i.width / 2 + l + s, x)
        );
      }),
      a
    );
  ...
  Hr = g(async (e, t) => {
    ...
    let f = 0;
    const w = t.classData.annotations?.[0],
      v = t.classData.annotations[0] ? '«' + t.classData.annotations[0] + '»' : '',
      L = await U(b, v, t.labelStyle, !0, !0);
    let _ = L.getBBox();
    if (F(R())) {
    ...
    t.classData.type !== void 0 &&
      t.classData.type !== '' &&
      (F(R()) ? (I += '&lt;' + t.classData.type + '&gt;') : (I += '<' + t.classData.type + '>'));
    const D = await U(b, I, t.labelStyle, !0, !0);
    ...
    ((u += m.height + i), m.width > o && (o = m.width));
    const d = [];
    (t.classData.members.forEach(async (E) => {
      const h = E.getDisplayDetails();
      let W = h.displayText;
      F(R()) && (W = W.replace(/</g, '&lt;').replace(/>/g, '&gt;'));
      const p = await U(b, W, h.cssStyle ? h.cssStyle : t.labelStyle, !0, !0);
      let O = p.getBBox();
      if (F(R())) {
    ...
    if (
      ...
      let E = (o - _.width) / 2;
      (T(L).attr('transform', 'translate( ' + ((-1 * o) / 2 + E) + ', ' + (-1 * u) / 2 + ')'),
        (f = _.height + i));
    }
    let k = (o - m.width) / 2;
    return (
      T(D).attr('transform', 'translate( ' + ((-1 * o) / 2 + k) + ', ' + ((-1 * u) / 2 + f) + ')'),
      (f += m.height + i),
      c
        .attr('class', 'divider')
        .attr('x1', -o / 2 - a)
      ...
      (f += l),
      d.forEach((E) => {
        T(E).attr('transform', 'translate( ' + -o / 2 + ', ' + ((-1 * u) / 2 + f + l / 2) + ')');
        const h = E?.getBBox();
        f += (h?.height ?? 0) + i;
      }),
      (f += l),
      ...
      (f += l),
      y.forEach((E) => {
        T(E).attr('transform', 'translate( ' + -o / 2 + ', ' + ((-1 * u) / 2 + f) + ')');
        const h = E?.getBBox();
        f += (h?.height ?? 0) + i;
      }),
      n
...
async function se(e, t, a, i, l) {
  const s = new me({ multigraph: !0, compound: !0 });
      ...
      if (n?.size && c?.size) {
        ...
        (gr(
          ...
          r.label &&
            ...
            cr({ ...r, x: u[1].x, y: u[1].y }, { originalPath: u })));
      }
    }
}
g(se, 'insertEdges');
var Ur = g(function (e, t) {
    return t.db.getClasses();
  }, 'getClasses'),
  Xr = g(async function (e, t, a, i) {
    const { securityLevel: l, block: s } = at(),
      r = i.db;
    let n;
    l === 'sandbox' && (n = T('#i' + t));
    const c = l === 'sandbox' ? T(n.nodes()[0].contentDocument.body) : T('body'),
      x = l === 'sandbox' ? c.select(`[id="${t}"]`) : T(`[id="${t}"]`);
    rr(x, ['point', 'circle', 'cross'], i.type, t);
    const u = r.getBlocks(),
      b = r.getBlocksFlat(),
      f = r.getEdges(),
      w = x.insert('g').attr('class', 'block');
    await re(w, u, r);
    const v = jt(r);
    if ((await ae(w, u, r), await se(w, f, b, r, t), v)) {
      const L = v,
        _ = Math.max(1, Math.round(0.125 * (L.width / L.height))),
        I = L.height + _ + 10,
        D = L.width + 10,
        { useMaxWidth: m } = s;
      (oe(x, I, D, !!m),
        S.debug('Here Bounds', v, L),
        x.attr('viewBox', `${L.x - 5} ${L.y - 5} ${L.width + 10} ${L.height + 10}`));
    }
  }, 'draw'),
  jr = { draw: Xr, getClasses: Ur },
  ga = { parser: ke, db: He, renderer: jr, styles: Ue };
export { ga as diagram };
