const e = {
  'azure.azureApiVersion.desc':
    'Azure の API バージョン、YYYY-MM-DD 形式に従う、[最新バージョン](https://learn.microsoft.com/zh-cn/azure/ai-services/openai/reference#chat-completions)を参照',
  'azure.azureApiVersion.fetch': 'リストを取得',
  'azure.azureApiVersion.title': 'Azure API Version',
  'azure.empty': 'モデル ID を入力して最初のモデルを追加してください',
  'azure.endpoint.desc':
    'Azure ポータルでリソースを確認する際に、「キーとエンドポイント」セクションでこの値を見つけることができます',
  ...
  'azure.token.desc':
    'Azure ポータルでリソースを確認する際に、「キーとエンドポイント」セクションでこの値を見つけることができます。KEY1 または KEY2 を使用できます',
  'azure.token.placeholder': 'Azure API Key',
  'azure.token.title': 'API Key',
  'azureai.azureApiVersion.desc':
    'AzureのAPIバージョン。YYYY-MM-DD形式に従い、[最新バージョン](https://learn.microsoft.com/zh-cn/azure/ai-services/openai/reference#chat-completions)を参照してください。',
  'azureai.azureApiVersion.fetch': 'リストを取得',
  'azureai.azureApiVersion.title': 'Azure APIバージョン',
  'azureai.endpoint.desc':
    'Azure AIプロジェクトの概要からAzure AIモデル推論エンドポイントを見つけます。',
  ...
  'bedrock.sessionToken.desc':
    'AWS SSO/STSを使用している場合は、AWSセッショントークンを入力してください。',
  'bedrock.sessionToken.placeholder': 'AWSセッショントークン',
  'bedrock.sessionToken.title': 'AWSセッショントークン（オプション）',
  ...
  'bedrock.unlock.description':
    'AWS AccessKeyId / SecretAccessKey を入力するとセッションを開始できます。アプリは認証情報を記録しません',
  'bedrock.unlock.imageGenerationDescription':
    'AWSのAccessKeyId / SecretAccessKeyを入力すると、生成を開始できます。アプリは認証情報を記録しません。',
  ...
  'cloudflare.baseURLOrAccountID.desc':
    'Cloudflare アカウント ID またはカスタム API アドレスを入力してください。',
  ...
  'comfyui.authType.options.bearer': 'Bearer（APIキー）',
  ...
  'createNewAiProvider.id.format':
    '数字、小文字のアルファベット、ハイフン（-）、およびアンダースコア（_）のみを含むことができます',
  'createNewAiProvider.id.placeholder':
    '小文字で入力してください（例: openai）。作成後は変更できません',
  ...
  'github.personalAccessToken.desc':
    'あなたのGithub PATを入力してください。[こちら](https://github.com/settings/tokens)をクリックして作成します',
  'github.personalAccessToken.placeholder': 'ghp_xxxxxx',
  'github.personalAccessToken.title': 'GitHub PAT',
  'huggingface.accessToken.desc':
    'あなたの HuggingFace トークンを入力してください。 [こちら](https://huggingface.co/settings/tokens) をクリックして作成します。',
  ...
  'ollama.customModelName.desc':
    'カスタムモデルを追加します。複数のモデルはカンマ（,）で区切ります',
  ...
  'ollama.download.desc':
    'Ollamaはこのモデルをダウンロードしています。このページを閉じないでください。再ダウンロードすると中断したところから再開されます。',
  'ollama.download.failed':
    'モデルのダウンロードに失敗しました。ネットワークまたはOllamaの設定を確認して再試行してください',
  ...
  'ollama.endpoint.desc':
    'http(s)://を含める必要があります。ローカルで特に指定がない場合は空白のままで構いません',
  ...
  'providerModels.config.checker.desc':
    'API キーとプロキシアドレスが正しく入力されているかテストします',
  ...
  'providerModels.config.fetchOnClient.desc':
    'クライアントリクエストモードはブラウザから直接セッションリクエストを発起し、応答速度を向上させます',
  ...
  'providerModels.config.responsesApi.desc':
    'OpenAI の次世代リクエスト形式仕様を採用し、思考チェーンなどの高度な機能を解放します（OpenAI モデルのみ対応）',
  ...
  'providerModels.item.customModelCards.confirmDelete':
    'このカスタムモデルを削除しようとしています。削除後は復元できませんので、慎重に操作してください。',
  ...
  'providerModels.item.modelConfig.reasoning.extra':
    'この設定は、モデルの深い思考能力を有効にするだけです。具体的な効果はモデル自体に依存しますので、このモデルが利用可能な深い思考能力を持っているかどうかはご自身でテストしてください。',
  'providerModels.item.modelConfig.reasoning.title': '深い思考をサポート',
  'providerModels.item.modelConfig.search.extra':
    'この設定はモデル内蔵の検索エンジンによるオンライン検索機能のみを有効にします。内蔵検索エンジンの対応可否はモデル自体に依存するため、このモデルの検索機能が利用可能かどうかをご自身でご確認ください。',
  ...
  'providerModels.item.modelConfig.type.extra':
    '異なるモデルタイプは、それぞれ異なる使用シーンと能力を持っています',
  ...
  'providerModels.item.modelConfig.video.extra':
    'この設定はアプリ内のビデオ認識機能のみを有効にします。認識の可否はモデル自体に依存するため、このモデルのビデオ認識機能が利用可能かどうかをご自身でご確認ください。',
  'providerModels.item.modelConfig.video.title': 'ビデオ認識対応',
  'providerModels.item.modelConfig.vision.extra':
    'この設定はアプリ内の画像アップロード設定のみを有効にします。認識のサポートはモデル自体に依存しますので、そのモデルの視覚認識機能の可用性を自分でテストしてください',
  ...
  'providerModels.list.empty.desc':
    'カスタムモデルを作成するか、モデルを取得してから使用を開始してください',
  ...
  'providerModels.list.enabledEmpty':
    '有効なモデルはありません。下のリストからお気に入りのモデルを有効にしてください〜',
  ...
  'providerModels.list.resetAll.conform':
    '現在のモデルのすべての変更をリセットしてもよろしいですか？リセット後、現在のモデルリストはデフォルトの状態に戻ります',
  ...
  'updateAiProvider.confirmDelete':
    'この AI サービスプロバイダーを削除しようとしています。削除後は復元できません。削除してもよろしいですか？',
  ...
  'vertexai.region.desc':
    'Vertex AI サービスのリージョンを選択してください。一部のモデル（例：Gemini 2.5）は特定のリージョン（例：global）でのみ利用可能です。',
